...
2. **Игрок уходит из отряда** → админ меняет статус на `left` → linked user role автоматически → `guest` (админов не трогает)
3. **Игрок возвращается** → админ меняет статус обратно (`active`/`reserve`) → linked user role автоматически → `member`
4. **Игрок сменил ник** → админ обновляет `nickname`, старый ник уходит в `nicknameHistory[]`, `nicknameIndex` обновляется автоматически
